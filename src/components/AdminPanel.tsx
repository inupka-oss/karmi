...
export default function AdminPanel({ userEmail, genres, initialAnime }: {
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/episodes?anime_id=eq.${id}`, {
      method: 'DELETE',
      headers: { 'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, 'Authorization': `Bearer ${accessToken}` },
    })
  ...
  const handleAddEpisode = async (animeId: string) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/episodes`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
  ...
  const handleDeleteEpisode = async (episodeId: string, animeId: string) => {
    const accessToken = getAccessToken()
    await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/episodes?id=eq.${episodeId}`, {
      method: 'DELETE',
      headers: { 'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, 'Authorization': `Bearer ${accessToken}` },
    })
