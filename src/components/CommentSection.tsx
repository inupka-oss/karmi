...
function CommentItem({ 
  ...
  return (
    <div className={`${isNested ? 'ml-4 sm:ml-8 border-l-2 border-white/10 pl-4' : ''}`}>
...
export default function CommentSection({ animeId }: { animeId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Используем RPC функцию save_anime_comment
...
export default function CommentSection({ animeId }: { animeId: string }) {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      // Загружаем все комментарии с реакциями
      const res = await fetch(
        `${supabaseUrl}/rest/v1/comments?anime_id=eq.${animeId}&select=*&order=created_at.desc`,
        { headers }
      )
      ...
      // Загружаем реакции текущего пользователя
      if (token) {
        const user = await loadCurrentUser()
        setCurrentUser(user)
        if (user) {
  ...
  return (
    <div className="mt-10">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3 mb-6">
        ...
        <button 
          type="submit" 
          className="bg-neo-pink hover:bg-neo-pink/80 text-white px-6 py-2 rounded-xl self-end transition"
        >
          Отправить
        </button>
      </form>
